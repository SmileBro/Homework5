...
func NewSignatureSha256FromFileSource(file *os.File, hashString string) (sig SignatureSha256, err error) {
	...
	sig.date = modTime.Truncate(time.Second)
...
func NewSignatureSha256FromString(text string) (sig SignatureSha256, err error) {
	sig = SignatureSha256{}
	sepText := strings.Split(text, separator)
	stat := strings.Split(sepText[0], ":")
	sig.date, err = time.Parse("2006-01-02 15-04-05", stat[0])
	if err != nil {
		fmt.Println(err)
		return SignatureSha256{}, err
	}
	sig.size = stat[1]
	sig.name = stat[2]
	sig.signature = []byte(sepText[1])
	fmt.Printf("sign line %x \n", sig.signature)
	return
}
...
func (s SignatureSha256) Equals(c contract.Signature) bool {
	if s.name != c.Name() {
		return false
	}
	if s.Size() != c.Size() {
